...
bool write_to_device(uint8_t dev_address, const uint8_t *src, uint8_t len)
{
    assert(len > 0);
    assert(src != NULL);
    int i = i2c_write_timeout_us(I2C_PORT, dev_address, src, len, false, I2C_TIMEOUT * len);
    if (i != len)
    {
        printf("I2C Error:%i\n", i);
        return false;
...
bool write_to_register(uint8_t dev_address, uint8_t dev_register, const uint8_t *src, uint8_t len)
{
    ...
    i = i2c_write_timeout_us(I2C_PORT, dev_address, src, len, false, I2C_TIMEOUT * len);
    if (i != len)
    {
        printf("I2C Error:%i\n", i);
        return false;
