...
#define BSEC_MAX_PROPERTY_BLOB_SIZE  (2005)     /*!< Maximum size (in bytes) of the data blobs returned by bsec_get_configuration() */
...
typedef enum
{
	...
	*/
     ...
     * In standard scan mode, the probability of H2S and non-H2S is provided by the variables BSEC_OUTPUT_GAS_ESTIMATE_1 & BSEC_OUTPUT_GAS_ESTIMATE_2 respectively. A maximum of 4 classes can be used by configuring with BME AI-studio.
     */	
    ...
    BSEC_OUTPUT_RAW_GAS_INDEX = 26,      /*!< Gas index cyclically running from 0 to ::heater_profile_len-1, range of heater profile length is from 1 to 10, default being 10 */      
